...
DEBUG = False                   # True -> only crawl DEBUG_PAGES pages
DEBUG_PAGES = 2
MANUAL_PAGE_COUNT_FALLBACK = 10  # used only if auto-detection of total results fails

OUTPUT_CSV = "cases_output.csv"
OUTPUT_XLSX = "cases_output.xlsx"
...
def map_columns(table) -> dict:
    """Return {'date': idx, 'judge': idx, 'case_num': idx} based on header row."""
    header_row = table.find("tr")
    mapping = {"date": None, "judge": None, "case_num": None}
    if not header_row:
        return mapping
...
def crawl(base_url: str, debug: bool) -> List[CaseRecord]:
...
def save_output(records: List[CaseRecord], csv_path: str, xlsx_path: str):
    ...
    fieldnames = ["case_number", "case_link", "date_of_receipt", "judge", "source_page"]
...
if __name__ == "__main__":
    ...
    results = crawl(BASE_URL, debug=debug_mode)
    save_output(results, OUTPUT_CSV, OUTPUT_XLSX)
    print("Done.")
